...
![img](https://ask.qcloudimg.com/http-save/yehe-7197959/5e3cd0cc6426a373ebae611e0def6b09.png?imageView2/2/w/2560/h/7000)
...
* binlog 是属于 MySQL Server 层的日志，MySQL 在完成一条更新操作后，Server 层还会生成一条 binlog，等之后事务提交的时候，会将该事物执行过程中产生的所有 binlog 统一写 入 binlog 文件。
* binlog 可以实现**主从复制**和**数据恢复**两个作用，但是不能左 crash safe
...
- 先写redo log，crash 后 bin log 备份恢复时少了一次更新，与当前数据不一致。
